...
export default function Intro() {
  return (
    ...
    <section className="w-full min-h-screen max-w-screen bg-background/70 flex flex-col justify-center items-center gap-8 xs:gap-12 px-4">
        <div className="text-white text-5xl  break-normal sm:text-6xl md:text-7xl flex flex-col justify-center items-center gap-4 font-syne font-extrabold uppercase">
            <p className='leading-[55px] xs:leading-[70px] text-center'>Play. Guess. <br/><span className="text-accent-green">Bond.</span>   </p>
        </div>

        <div className="max-w-lg text-text-primary text-base xs:text-lg">
            <p>
        ...
        <div className="flex flex-col sm:flex-row font-semibold justify-center items-center gap-3 xs:gap-6">
            ...
            <a href='/signin' className="text-center gap-2.5 flex justify-center items-center hover:shadow-lg transition-all duration-200 shadow-accent-blue/40 w-72 max-w-[90vw] p-5 xs:p-6 sm:p-0 cursor-pointer sm:min-h-14 text-white rounded-xl bg-accent-blue">
                <BsGlobe size={22}/>
                Play on web
            </a>
        </div>
