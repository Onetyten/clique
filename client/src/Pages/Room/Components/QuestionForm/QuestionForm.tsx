...
export default function QuestionForm({setShowQuestionForm,questionLoading,setQuestionLoading}:propType) {
  ...
  return (
    <div onClick={()=>setShowQuestionForm(false)} className="absolute w-full z-20 flex justify-center items-center h-full bg-background/70">
        <div onClick={(e)=>e.stopPropagation()} className="p-6 min-h-80 sm:p-8 w-10/12 sm:w-md bg-background text-sm text-text-primary rounded-sm flex flex-col items-center gap-5">
            ...
            {questionLoading?(
                ...
                <form onSubmit={askQuestion} className="flex flex-col gap-3 w-full">
                    ...
                    <div className="flex items-center w-full gap-2">
                        <button type="submit" disabled={questionLoading} id="ask-btn" className="p-3 sm:p-3.5 w-full flex justify-center items-center relative disabled:bg-text-muted bg-accent-green hover:bg-accent-green/80 text-background font-bold rounded-sm">
                            Ask
                        ...
                        <button type="reset" onClick={()=>setShowQuestionForm(false)} className="p-3 sm:p-3.5 w-full disabled:bg-text-muted bg-red-500 hover:bg-red-500/80 text-background font-bold rounded-sm">
                            Cancel
                        </button>
                    </div>
                </form>
