...
export async function handleAskQuestion(socket: Socket, { user,question,answer, endTime }: QuestionType) {
  ...
  const adminId = await redis.get('adminId')
  if (!adminId) {
  ...
  await pool.query('INSERT INTO sessions (is_active,room_id,gm_id,question,answer,end_time) values ($1,$2,$3,$4,$5,$6)',[true,user.room_id,user.id,question,answer,endTime])
